...
Образ контейнера для `docker`. Роль вычисляет его сама (пин или сборка из репозитория и версии); как задать и какой сейчас пин — таблица переменных в `config/README.md`.
...
Идентификатор клиента, который VPN-клиент шлёт в каждом пакете. После ротации меняется. На сервере хранится в `reality-state.json` как массив.
...
`xrayvpn` без аргументов (или `xrayvpn repl`): команды того же синтаксиса, что и CLI, вводятся
без префикса; `exit` возвращает в shell. Прежнее имя — «REPL».

## Portable-сборка (standalone)

Единый исполняемый файл из GitHub Releases со встроенными ролями и плейбуками; Python и клон
репозитория не нужны.

## Staging-директория деплоя

`/tmp/xrayvpn-<run-id>` на VPS: распакованный репозиторий, временный inventory, скачанные конфиги.
Старые прогоны вычищаются на следующем запуске; `--no-cleanup` оставляет staging, `--full-cleanup` сносит и venv.
...
Все параметры `config/settings.yml` описаны в `config/README.md`.
